...
// Figma assets for AI Mechanic Panel - Updated to match Figma design node 131:1308
const iconBot = (
...
const DEFAULT_MESSAGE: Message = {
    ...
    content: "",
    summary:
        "SkyMaintain AI Mechanic initialized. This decision support system provides maintenance guidance based on FAA/EASA regulations, manufacturer documentation, and engineering best practices.",
    recommendation:
        "Ask questions about aircraft systems, maintenance procedures, regulatory compliance, or predictive maintenance. All guidance is advisory only—final authority remains with certified maintenance personnel.",
    confidence: 100,
    references: [
        {
    ...
    manuals.forEach((manual, idx) => {
        ...
        if (
            ...
            alerts.push({
                ...
                description: `AI Mechanic recommends reviewing ${manual.filename} for upcoming scheduled maintenance items based on current flight hours.`,
...
export default function AIMechanicPanel({
    ...
    React.useEffect(() => {
        ...
        setMessages((prev) => {
            ...
            if (first.summary?.startsWith("SkyMaintain AI Mechanic initialized")) {
                return [
                    {
                        ...first,
                        references: [
    ...
    return (
        <div
            ref={panelRef}
            className="fixed z-[100] flex flex-col overflow-hidden rounded-[14px]"
            style={{
                width: "480px",
                maxHeight: "600px",
                right: "24px",
        ...
        >
            ...
            >
                <div className="flex items-start gap-3">
                    ...
                    >
                        <div className="relative h-6 w-6 text-[#155dfc]">{iconBot}</div>
                        <div
                            className="absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full border-[1.6px] border-white opacity-50"
                    ...
                    <div className="flex flex-col gap-2">
                        <div className="flex items-center gap-2">
                            <span className="text-lg font-bold text-white" style={{ fontFamily: "Arial, sans-serif" }}>
                                SkyMaintain AI Mechanic
                            </span>
                            {/* Info icon */}
                            <div className="h-4 w-4 opacity-80">
                                { }
                ...
                <div className="flex gap-2">
                    {onMinimize && (
                        <button
                            ...
                            className="flex h-8 w-9 items-center justify-center rounded-lg hover:bg-white/20 transition-colors"
                            title="Minimize"
                        >
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            <img src={imgIconExpand} alt="" className="h-4 w-4" />
                        </button>
                    )}
                    <button
                        type="button"
                        onClick={onClose}
                        aria-label="Close AI Mechanic (ESC)"
                        className="flex h-8 w-9 items-center justify-center rounded-lg hover:bg-white/20 transition-colors pointer-events-auto"
                        title="Close (ESC)"
                    >
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={imgIconClose} alt="" className="h-4 w-4" />
                    </button>
                </div>
            </div>
            ...
            <div
                className="flex gap-3 border-b px-4 py-3 items-start"
            ...
            >
                ...
                >
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={imgIconInfo} alt="" className="h-4 w-4" />
                </div>
                <div className="flex-1 text-sm leading-relaxed" style={{ color: "#1e2939" }}>
                    <span className="font-bold">
                        SkyMaintain AI Mechanic uses only the manuals you are authorized to
                        upload.
                    </span>{" "}
                    All answers are grounded in your documents with revision traceability.
                </div>
                <button
                    type="button"
                    onClick={onClose}
                    aria-label="Dismiss"
                    className="flex h-8 w-9 flex-shrink-0 items-center justify-center rounded-lg hover:bg-blue-100 transition-colors pointer-events-auto"
                    title="Dismiss"
                >
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={imgIconDismiss} alt="" className="h-4 w-4" />
                </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4" style={{ backgroundColor: "#f9fafb" }}>
                {messages.map((message, idx) => (
                    <div key={idx} className={`flex gap-3 mb-4 ${message.type === "user" ? "justify-end" : ""}`}>
                        {/* Bot Avatar */}
                        {message.type === "assistant" && (
                            ...
                            >
                                <div className="h-4 w-4 text-[#155dfc]">
                                    <svg
                                        width="16"
                                        height="16"
                                        viewBox="0 0 16 16"
                        ...
                        <div
                            className={`rounded-[14px] border-[1.6px] p-4 ${message.type === "user" ? "max-w-[75%]" : "flex-1"}`}
                            style={{
                        ...
                        >
                            ...
                            {message.summary && (
                                <div className="mb-4">
                                    <div className="mb-2 flex items-center gap-2">
                                        {/* eslint-disable-next-line @next/next/no-img-element */}
                                        <img src={imgIconSummary} alt="" className="h-4 w-4" />
                                        <span
                            ...
                            >
                                {/* eslint-disable-next-line @next/next/no-img-element */}
                                <img src={imgIconEvidence} alt="" className="h-4 w-4" />
                                <span
                                    className="text-sm font-bold"
                                    style={{ color: "#101828" }}
                                >
                                ...
                                <img
                                    src={imgIconChevron}
                                    alt=""
                                    className={`h-4 w-4 transition-transform duration-200 ${showEvidence ? "rotate-180" : ""}`}
                                />
                            </button>
                            ...
                            {showEvidence && message.recommendation && (
                                <div
                                    className="mb-4 rounded-[4px] border-l-4 p-3 pl-4"
                                    style={{
                                ...
                                >
                                    <div className="mb-2 flex items-center gap-2">
                                        {/* eslint-disable-next-line @next/next/no-img-element */}
                                        <img src={imgIconRecommend} alt="" className="h-4 w-4" />
                                        <span
                                            className="text-sm font-bold"
                                            style={{ color: "#101828" }}
                                        >
                            ...
                            {showEvidence && message.references && message.references.length > 0 && (
                                <div>
                                    <div className="mb-2 flex items-center gap-2">
                                        {/* eslint-disable-next-line @next/next/no-img-element */}
                                        <img src={imgIconRef} alt="" className="h-4 w-4" />
                                        <span
                                            className="text-sm font-bold"
                                            style={{ color: "#101828" }}
                                        >
                                    ...
                                    {message.references.map((ref, refIdx) => (
                                        ...
                                        >
                                            ...
                                            >
                                                {ref.url}
                                                {/* eslint-disable-next-line @next/next/no-img-element */}
                                                <img src={imgIconExternal} alt="" className="h-3 w-3" />
                                            </a>
            ...
            >
                <p className="mb-2 text-xs" style={{ color: "#4a5565" }}>
                    Suggested queries:
                </p>
                <div className="flex flex-wrap gap-2">
                    {SUGGESTED_QUERIES.map((query) => (
                        <button
                            ...
                            className="rounded-lg border-[0.8px] px-3 py-2 text-xs transition-colors hover:bg-gray-50 active:bg-gray-100"
                            style={{
            ...
            >
                <div className="flex gap-2 items-center">
                    <input
                        ...
                        className="flex-1 rounded-lg border-[0.8px] px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                        style={{
                    ...
                    <button
                        ...
                        onClick={handleSend}
                        className="flex h-9 w-10 items-center justify-center rounded-lg transition-all disabled:opacity-50 hover:bg-blue-700 active:scale-95"
                        style={{ backgroundColor: "#155dfc" }}
                        title="Send message"
                    >
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={imgIconSend} alt="" className="h-4 w-4" />
                    </button>
                    {/* Mic/Voice Button - Matches Figma node 131:1458 */}
                    <button
                        type="button"
                        onClick={handleClear}
                        className="flex h-8 w-9 items-center justify-center rounded-lg border-[0.8px] transition-colors hover:bg-gray-50 active:bg-gray-100"
                        style={{ borderColor: "rgba(0,0,0,0.1)", color: "#364153" }}
                        title="Clear conversation"
                    >
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={imgIconMic} alt="" className="h-4 w-4" />
                    </button>
                </div>
            </div>
        </div>
